...
#if defined(NDEBUG)
# error "ZUGCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3818f6da5c6db6244de83671bdbf25ecd61664e9cde530998cd4a5e64d59df0e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ZUGCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ZUGCoin Signed Message:\n";
...
{
    // ZUGCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ZUGCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ZUGCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1527606343, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "2018 05 29";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041e6d765ae9b33212fff42d54aac988448e19f5c92bc6bb72a082cca14102901e07d18a2a1342c79d4313b02557c6666574f3924f2d9462be2d46dcd8bbfb8bf9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527606343;
        ...
        assert(block.hashMerkleRoot == uint256("0x4b4a39aafefcad5fcbf5d521a6ddcfbb3b33bbc491ccaa1de31a856091330352"));
...
unsigned char pchMessageStart[4] = { 0x08, 0xcb, 0x6a, 0x3b }; // ZUGCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZUGCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZUGCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZUGCoinMiner(CWallet *pwallet)
{
    printf("ZUGCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zugcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ZUGCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZUGCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZUGCoinMiner, pwallet));
}
